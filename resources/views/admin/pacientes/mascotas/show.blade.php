<!-- resources/views/paciente/mascotas/show.blade.php -->
                ...
                <div class="space-y-2">
                    ...
                    <p class="text-indigo-100">
                        @if($mascota->sexo == 'Macho')
                            <span class="text-blue-100">♂ Macho</span>
                        @elseif($mascota->sexo == 'Hembra')
                            <span class="text-pink-100">♀ Hembra</span>
                        @else
                            <span class="text-gray-100">No especificado</span>
                        @endif
                        · {{ $mascota->especie }} {{ $mascota->raza ? '· '.$mascota->raza : '' }}
                    </p>
                </div>
                <div class="flex flex-wrap gap-3 mt-4 md:mt-0">
                    <a href="{{ route('paciente.mascotas.index') }}" class="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm text-white px-4 py-2 rounded-lg hover:bg-white/20 transition-all duration-200">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                        </svg>
                        Volver al listado
                    </a>
                    <a href="{{ route('paciente.mascotas.edit', $mascota) }}" class="inline-flex items-center gap-2 bg-white text-indigo-700 px-4 py-2 rounded-lg shadow-sm hover:bg-indigo-50 transition-all duration-200">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-lg p-6 transform transition-all duration-300 hover:shadow-xl">
                    ...
                    @if(isset($mascota->proximaVacuna) || isset($mascota->proximaDesparasitacion))
                        <div class="space-y-4">
                            @if(isset($mascota->proximaVacuna))
                            ...
                            @if(isset($mascota->proximaDesparasitacion))
                                <div class="bg-gradient-to-r from-green-50 to-teal-50 p-4 rounded-xl border border-green-100">
                                    <div class="flex">
                                        ...
                                        <div class="ml-4">
                                            ...
                                            <p class="mt-1 text-green-600">{{ $mascota->proximaDesparasitacion->producto }}</p>
        ...
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden mb-8">
                ...
                @if(count($mascota->vacunas) > 0)
                    <div class="overflow-x-auto rounded-lg">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            @foreach($mascota->vacunas->sortByDesc('fecha_aplicacion') as $index => $vacuna)
                                <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-gray-50' }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        @if($vacuna->fecha_proxima)
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                                {{ \Carbon\Carbon::parse($vacuna->fecha_proxima)->format('d/m/Y') }}
                                            </span>
                                        @else
                ...
                @if(count($mascota->desparasitaciones) > 0)
                    <div class="overflow-x-auto rounded-lg">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                            <tr>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider rounded-tl-lg">Producto</th>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Aplicada</th>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Próxima</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($mascota->desparasitaciones->sortByDesc('fecha_aplicacion') as $index => $desparasitacion)
                                <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-gray-50' }}">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        {{ $desparasitacion->producto }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ \Carbon\Carbon::parse($desparasitacion->fecha_aplicacion)->format('d/m/Y') }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        @if($desparasitacion->fecha_proxima)
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                                {{ \Carbon\Carbon::parse($desparasitacion->fecha_proxima)->format('d/m/Y') }}
                                            </span>
                                        @else
                                            <span class="text-gray-400">No programada</span>
                                        @endif
            ...
            <div id="visitas-tab" class="tab-content hidden p-6">
                @if(count($mascota->visitas) > 0)
                    <div class="space-y-4">
                        @foreach($mascota->visitas->sortByDesc('fecha_visita') as $visita)
                            <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                                    ...
                                    <div class="flex justify-between items-center">
                                        <h4 class="font-medium text-indigo-700">
                                            ...
                                            @if($visita->hora_visita)
                                                <span class="text-indigo-500 ml-2">{{ \Carbon\Carbon::parse($visita->hora_visita)->format('H:i') }} hrs</span>
                                            @endif
                                        </h4>
                                ...
                                <div class="p-4">
                                    <div class="mb-3">
                                        <span class="text-xs font-medium text-gray-500">Motivo de consulta:</span>
                                        <p class="text-gray-700">{{ $visita->motivo_visita ?? $visita->motivo_consulta }}</p>
                                    </div>

                                    @if($visita->diagnostico)
                                        <div class="mb-3">
                                            <span class="text-xs font-medium text-gray-500">Diagnóstico:</span>
                                            <p class="text-gray-700">{{ $visita->diagnostico }}</p>
                                        </div>
                                    @endif

                                    @if($visita->tratamiento)
                                        <div class="mb-3">
                                            <span class="text-xs font-medium text-gray-500">Tratamiento:</span>
                                            <p class="text-gray-700">{{ $visita->tratamiento }}</p>
                                        </div>
                                    @endif

                                    <div>
                                        <span class="text-xs font-medium text-gray-500">Atendido por:</span>
                                        <p class="text-gray-700">{{ $visita->doctor ? $visita->doctor->nombre : 'No registrado' }}</p>
                                    </div>
                                </div>
                            </div>
                        @endforeach
