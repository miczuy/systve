...
<head>
    ...
    <title>Informe Médico: Listado de Pacientes</title>
    <style>
...
<div class="content">
    ...
    <div class="title-section">
        ...
        <p>Personal médico activo: <span class="highlight-text">1</span> pacientes</p>
                ...
                @foreach($pacientes as $paciente)
                    <tr>
                        <td class="patient-id-cell">
                            <span class="patient-id">{{ $loop->iteration }}</span>
                        </td>
                        <td class="patient-info-cell">
                            <span class="patient-name">{{ $paciente->nombres }} {{ $paciente->apellidos }}</span>
                            <span class="patient-title">Paciente</span>
                        </td>
                        <td>{{ $paciente->cedula }}</td>
                        <td>
                            <span class="email-text">{{ $paciente->correo }}</span>
                            @if($paciente->telefono)
                                <span class="email-text">{{ $paciente->telefono }}</span>
                            @endif
                        </td>
                        <td>{{ $paciente->direccion }}</td>
                        <td>{{ $paciente->fecha_nacimiento ? date('d/m/Y', strtotime($paciente->fecha_nacimiento)) : 'No disponible' }}</td>
                        <td>
                            <span class="info-badge">{{ $paciente->sexo }}</span>
                            <span class="info-badge">{{ $paciente->edad }} años</span>
                        </td>
                    </tr>
                @endforeach
            @else
    ...
    <div class="stats-row">
        <div class="stat-column">
            <div class="stat-box stat-blue">
                ...
                <span class="stat-value">{{ $totalPacientes ?? 0 }}</span>
            </div>
        </div>
        <div class="stat-column">
            <div class="stat-box stat-purple">
                <span class="stat-label">Edad Promedio</span>
                <span class="stat-value">{{ $edadPromedio ?? 0 }} años</span>
            </div>
        </div>
        <div class="stat-column">
            <div class="stat-box stat-green">
                <span class="stat-label">Distribución por Sexo</span>
                <span class="stat-value">{{ $distribucionGenero ?? 'Sin datos' }}</span>
                @if(isset($masculino) && isset($femenino) && ($masculino + $femenino > 0))
                    <small style="display: block; font-size: 8px; margin-top: 2px;">
                        Hombres: {{ $masculino }} | Mujeres: {{ $femenino }}
                    </small>
                @endif
            </div>
        </div>
        <div class="stat-column">
            <div class="stat-box stat-amber">
                ...
                <span class="stat-value">{{ $ultimoRegistro ?? date('d-m-Y') }}</span>
            </div>
        </div>
    </div>
