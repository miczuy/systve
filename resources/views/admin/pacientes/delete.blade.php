    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="w-full max-w-lg bg-white shadow-2xl rounded-lg overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-indigo-500 to-purple-500 text-white py-5 px-6 rounded-t-lg text-center">
                <h2 class="text-3xl font-bold mb-1">Eliminar Paciente</h2>
                <p class="text-lg">Estás a punto de eliminar al paciente: <span class="font-semibold">{{ $paciente->nombres }} {{ $paciente->apellidos }}</span></p>
            </div>

            <!-- Resumen Visual -->
            <div class="border border-gray-300 rounded-lg m-6 p-4">
                <h3 class="text-lg font-semibold text-gray-700 mb-2">Resumen del Paciente</h3>
                <ul class="space-y-2">
                    <li><strong>Nombre:</strong> {{ $paciente->nombres }} {{ $paciente->apellidos }}</li>
                    <li><strong>Cédula:</strong> {{ $paciente->cedula }}</li>
                    <li><strong>Fecha de Nacimiento:</strong> {{ $paciente->fecha_nacimiento ? date('d/m/Y', strtotime($paciente->fecha_nacimiento)) : 'No registrada' }}</li>
                    <li><strong>Celular:</strong> {{ $paciente->telefono }}</li>
                    <li><strong>Dirección:</strong> {{ $paciente->direccion }}</li>
                    <li><strong>Email:</strong> {{ $paciente->correo }}</li>
                </ul>
            </div>

            <!-- Formulario para eliminar -->
            <form id="eliminar-paciente-form" action="{{ url('/admin/pacientes', $paciente->id) }}" method="POST" class="m-6">
                @csrf
                @method('DELETE')
                <fieldset class="border border-gray-300 rounded-md p-4">
                    <legend class="text-xl font-medium text-gray-700 text-center">Confirmación</legend>
                    <p class="text-gray-600 mb-4 text-center">¿Estás seguro de que deseas eliminar este paciente?</p>
                </fieldset>

                <div class="mt-6 flex justify-center space-x-4">
                    <!-- Botón de cancelar -->
                    <a href="{{ url('admin/pacientes') }}" class="px-4 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600 transition flex items-center space-x-2">
        ...
        function confirmDelete() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    document.getElementById('eliminar-paciente-form').submit();
                }
            });
        }
    </script>
